...
- **Not measured this pass**: Core Web Vitals with real network/CPU throttling. This session's tooling had no throttle control, and per the item's own gotcha, localhost numbers are meaningless — needs a throttled Lighthouse/PageSpeed Insights run against the deployed Vercel preview before item 27 can move from `partial` to `done`.
