...
**Trust-strip differentiators rebuilt as a dark fold-open list — merged to `main` via [PR #33](https://github.com/akashsst-code/supertooth-dentistry/pull/33), reviewed and approved by Akash:** Akash flagged that the "Same-day appointments"/"Same-day crowns" mobile photo-cards looked identical to the (already tappable) "In-network" card next to them but did nothing on tap — a scroll-vs-tap mismatch. Went through two rounds: an interim horizontal photo-card carousel (with a stronger tap affordance and scroll-position dots) was mocked up alongside a vertical fold-open list and shown to Akash for a real decision (both options built as a live, tappable comparison artifact); Akash picked the vertical list.

Shipped: `TrustBlock.tsx`'s 3 differentiators now render as a single vertical accordion list at every breakpoint (no more separate mobile-carousel/desktop-grid split) under a new "Why Choose Us" heading, all 3 titles visible with zero interaction. Collapsed rows are compact icon+title+one-line-detail bars; expanding one reveals its real photo, a short note, and a shared "Book Appointment" + phone CTA routing to `/contact` (the real appointment path already used site-wide — the actual Tab32 booking widget still isn't built, see `BookingBlock.tsx`, so this doesn't fake one). The insurance row keeps its existing carrier-grid expanded content, restyled to match.

Card styling (dark espresso surface, warm-ivory text, translucent terracotta-on-ivory icon badges, plus a subtle two-tone radial-gradient wash for depth) echoes `Hero.tsx`'s dark panel per Akash's explicit "nicer feel like page 1" call, rather than the light warm-ivory cards used elsewhere on the page. Extracted the insurance card's accordion shell into a shared `ExpandCard` component (`InsuranceExpandCard.tsx`) so all 3 rows' interaction and styling can't drift apart.

Also fixed a real spacing bug surfaced by this pass: several short UI text lines (`<p>` tags with no explicit margin utility) were silently inheriting the site-wide `p { margin-bottom: 2em }` accessibility rule meant for long-form body copy, inflating one-line card labels into much taller blocks than their content needed — zeroed those out specifically rather than touching the global rule.
