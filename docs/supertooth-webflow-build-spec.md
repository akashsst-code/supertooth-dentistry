...
- **Services (`ServicesSection`)** — rebuilt per round-2 feedback to match smilemakersfortworth.com's pattern: trimmed from 6 to exactly 4 items (`content.ts`), single-column stack (not a grid) with a big image tile above compact title/description text, no links of any kind (dropped the earlier "Learn more" — no click-throughs for now), and a vertical connecting thread + node dot running through the 4 stacked cards, same visual language as the testimonials rail. No real per-service photography exists, and stock photography isn't appropriate for this site's real-photography-only trust convention, so each "image" is a large tinted icon tile sized to occupy roughly half the mobile viewport height (Akash's general guideline, modeled on the reference site: "pictures take half screen... on mobile, then some words") — swap in real photography later without changing the layout.
