...
**Insurance/offer block (homepage Section 3) — PR open, not yet merged:** the hero (Section 1) and trust block (Section 2) surface insurance carriers and reviews, but neither actually answers the insurance-driven segment's core question named in `supertooth-priority-dimensions.md` Section 1 ("do you take my plan") or represents "use-it-or-lose-it timing" from the same doc — nothing built so far did. Added an "Insurance questions, answered" panel with two direct Q&A pairs (plan acceptance; benefit-year timing, phrased generically so it isn't an unverifiable practice-specific claim) — also the first AEO-structured FAQ content anywhere on the homepage, per the locked Build Constraints. Also fixed the offer cards to match Section 3's own spec text literally: $149 new-patient offer is now the featured/primary card, $500 off Invisalign renders as the "secondary line" the spec calls for instead of an identical-weight card. No design tokens changed. FAQPage schema markup (Section 7 checklist) still not wired up — this only gets the content into the right structural shape for it.
