...
**Homepage flow below the office scroll reworked — PR open, not yet merged:** Akash reviewed smilemakersfortworth.com's homepage flow (testimonials-forward, dentist-bio credentials, service cards, community/map section, no on-page insurance) and asked for a set of homepage changes below the Trust block's office-photo carousel, keeping everything through that carousel unchanged. New order: TrustBlock (unchanged through the carousel, now closes with a brief "about our office" line) → InsuranceBlock (redesigned, kept in its existing earlier position) → TestimonialsSection (new) → ServicesSection (new) → CredentialsSection (new) → LocationMapSection (new) → NewPatientOffersBlock (new-patient offers, moved to last on purpose) → BookingBlock → Footer.
...
- **Insurance** split off from the old combined `InsuranceOfferBlock` into its own `InsuranceBlock` — same position in the flow, redesigned from plain check+text pills into typographic "wordmark badge" cards. Real trademarked carrier logo files aren't available/licensed for use here, so this is a deliberate styled-lettering design choice standing in for real logos, not scraped images — swap in real logo assets later if Akash sources licensed ones.
- **New-patient offers** split into its own `NewPatientOffersBlock`, moved to the very end of the flow per Akash's explicit "keep it last" call.
- **Testimonials, services, credentials, and map are all new.** Testimonials quote cards and Dr. Archana's credential/affiliation badges are structural placeholders only — real patient testimonials can't be invented (HIPAA / no-unverifiable-claims, Section 7) and Dr. Archana's actual professional-org memberships aren't confirmed yet, so both render through `<Placeholder>` pending real content from Akash. The services teaser reuses the two already-real differentiators (same-day appointments, same-day crowns) and placeholders the rest pending the practice's confirmed service list (Section 8). The map embeds the real, already-confirmed practice address (standard Google Maps iframe, no API key needed); the "areas we serve" list only confirms Queen Anne as real, the rest are proximity-based placeholders pending Akash's confirmation.
