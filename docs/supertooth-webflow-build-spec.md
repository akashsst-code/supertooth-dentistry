...
**Tap-to-expand insurance signals — merged to `main` via [PR #22](https://github.com/akashsst-code/supertooth-dentistry/pull/22), reviewed and approved by Akash:** two insurance touchpoints made interactive, referencing a competitor site's INSURANCE FAQ accordion Akash shared as a pattern. The Hero's "In-network: Delta, Premera, Aetna + more" line (`InsuranceTeaser.tsx`, client component) now completes the sentence in place on tap with the remaining `insuranceCarriers` — derived by matching short teaser names against the full carrier names rather than hardcoding a second list. The "In-network with most plans" differentiator card in `TrustBlock.tsx` (`InsuranceExpandCard.tsx`) got a +/- accordion revealing the full carrier list, animated open/closed via a CSS grid-rows transition with a plus-morphs-to-minus icon.

First pass wrapped each carrier name in `<Placeholder>` (dashed underline) inside the expanded card — Akash flagged that as visually unappealing and reading like broken links. Redesigned to reuse the typographic "wordmark badge" language already established in `InsuranceBlock.tsx` (display-italic name + soft terracotta accent circle, no underline) at a compact 2-column scale, with a single plain-text disclaimer line ("Don't see your plan? Call us and we'll verify.") carrying the compliance intent instead of per-name brackets — same tradeoff already made for the Hero teaser, which was never wrapped in `<Placeholder>` either. `insuranceCarriers` itself is unchanged and still unconfirmed against the practice's real network status (see `content.ts`); no carrier names from the reference screenshot were copied in, since that list belongs to a different practice.
