...
**Hero redesigned photo-first, full-bleed — merged to `main` 2026-08-30 via [PR #37](https://github.com/akashsst-code/supertooth-dentistry/pull/37), reviewed and approved by Akash:** replaces the split video/text layout (Hero v2, locked 2026-08-22) per Akash's ask, referencing 2thstudio.com's clean full-bleed pattern: "make the photo occupy most the screen, and bleed the bottom part in." `HeroCarousel.tsx` now fills the entire hero section edge-to-edge (was a 60%-wide side panel next to a solid espresso text panel) at every breakpoint; eyebrow/headline/body/trust-strip/CTAs are overlaid directly on the photo, bottom-anchored. The carousel's own internal scrim was removed since Hero.tsx now owns legibility contrast for the whole section (it was the only place HeroCarousel is used). Section 1's locked requirement — insurance signal + both CTAs visible on load, not buried — still holds; they're just overlaid on the photo now instead of living in a separate side panel.

Two rounds of review feedback after the first pass: (1) the initial thin all-over gradient scrim (`from-espresso/95 via-espresso/35`) left text sitting directly on busy photo detail with too little contrast — the scrim now uses the old panel's actual solid espresso color instead, and (2) that same first pass also darkened photo well above the text (a `via-60%`/`to-100%` gradient reached into the subject's face), so the stops are now tightened (`from-espresso from-0% via-espresso/85 via-38% to-transparent to-52%`) to confine the darkening to roughly where the "Accepting new patients" badge and copy actually sit, leaving the top half of the photo fully clear. The bottom of the hero was originally a soft fade-to-warm-ivory band meant to "bleed" into TrustBlock, but that read as a hazy smear rather than a deliberate transition — it's now a clean, sharp cut straight into the next section. `Hero.tsx`, `HeroCarousel.tsx`.
