...
Of the nine researched funnel stages, eight have a serving surface; post-visit has none, but that's already tracked as item 22. Two real gaps had no existing owner and are newly logged: **item 58** ("Add related-page cross-links between Insurance, Services and About," P1) — the insurance ↔ services ↔ about cross-links blueprint §20 calls for don't exist anywhere, only the nav connects them — and **item 59** ("Open question: native 'Get Directions' link vs. the on-page map embed," P2) — both map components deliberately sandbox their embed against navigating away (a past explicit call to keep visitors on-page), which means no tap on the site opens a native maps app; flagged as a `conflict` for Akash to rule on rather than reversed unilaterally. Of the 12 researched patient scenarios walked end to end, 9 complete cleanly; the other 3 stall on items 58, 59, 42 and 43 (before/after evidence and a languages-spoken signal), all already tracked or newly logged here. Full findings in item 55's `evidence` field in `src/lib/backlog.ts`.
