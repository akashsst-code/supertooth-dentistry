...
**Emergency-line number confirmed — merged to `main`:** the `contact.emergencyPhone` discrepancy flagged in PR #16 (two different numbers on the source site) is resolved — Akash confirmed the top-banner number `(206) 593-3131` is correct, the footer-box number `(206) 555-0199` was wrong. No longer behind `<Placeholder>` in `BookingBlock.tsx`/`Footer.tsx`.
