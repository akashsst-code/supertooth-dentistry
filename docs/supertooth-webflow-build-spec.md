...
**`/services` page built — PR open, not yet merged:** the "Services" nav link has pointed at `/services` since the nav was built, but the page itself never existed (404) — this resolves that gap, and is separate from the homepage's `ServicesSection` teaser, which stays exactly as locked (4 items, no links). New `serviceCategories` data in `content.ts` covers the fuller catalog — General Dentistry, Cosmetic Dentistry, Restorative Dentistry, Orthodontics, Other Treatments — sourced from the practice's existing site (gray-rail-265889.hostingersite.com/services), the same trusted source already used for contact/hours/Archana's bio. The 3 categories with real marketing photography in `public/services/` reuse those crop files; the rest render text-only rather than inventing a stock-photo or generic-icon stand-in.

Two follow-ups flagged in earlier passes are resolved here: Botox now has its own placement — a one-line pairing (`archana.botoxNote`) next to Dr. Archana's existing "Certified Botox Provider" credential in `TrustBlock.tsx` — and Invisalign, which already had a placement via the existing $500-off offer card, now also appears as its own entry under Orthodontics. Neither the Invisalign badge nor the Botox before/after photo Akash previously mentioned ever actually landed in `public/`, so both stay text/stock-photo only pending those files.

**Emergency dentistry:** listed as its own callout on `/services` (`emergencyService` in `content.ts`), separate from the regular category cards — per Akash's explicit call, it doesn't route into the normal `/contact` booking link like everything else; it calls the practice's main line directly instead, with a note that a dedicated online scheduling path is coming later. This does not reintroduce the standalone emergency phone line Akash had pulled off the site earlier (PR #19/#20) — it uses `contact.phone`, the same main number already public in the Nav and Footer.

**`feature/services-page` (PR #5), flagged as stale below, is now superseded by this page** and built fresh off current `main` instead of rebased — that branch predates the office-carousel/hero-carousel/icons work and would have deleted real content if merged. Worth closing PR #5 once this one is up, rather than leaving both open.
