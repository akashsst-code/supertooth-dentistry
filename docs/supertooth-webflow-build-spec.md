...
**Logo refresh — merged to `main` 2026-08-24 via [PR #17](https://github.com/akashsst-code/supertooth-dentistry/pull/17), reviewed and approved by Akash:** supersedes the plain-silhouette mark described below. New mark is a tooth silhouette with a terracotta sparkle accent, from artwork Akash supplied — cropped from the source file and background-keyed to transparent (`public/logo/tooth-mark-bold.png`) rather than redrawn as an SVG path, so the silhouette (including the cut-away lobe the sparkle sits behind) is exact. Wordmark changed from heavy italic to regular-weight Fraunces for "Supertooth"; "Dentistry" in tracked caps beneath is unchanged. Everything else about the lockup (placement, `aria-label` carrying the canonical `practice.name`, no location line in the header) is unchanged from the original decision below.
