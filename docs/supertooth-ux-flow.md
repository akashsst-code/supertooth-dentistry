...
First used in Hero.tsx's mobile layout (2026-08-29), replacing a stacked photo-block + solid-Espresso-panel layout that read as cluttered/two-screens-stacked on review. Desktop's side-by-side split is a separate, already-working pattern and isn't affected by any of this.

Several adjacent ideas were tried the same day and reverted — worth recording so they aren't re-tried the same way later:
- A gradient fade at the Hero/TrustBlock seam (softening Hero's solid bottom edge into TrustBlock's lighter Sand tint) — reverted, "I don't like the transition, make a clean split." A crisp flat-color boundary reads as a deliberate section break; the gradient read as muddy and indecisive. Not every section transition needs to bleed — only photo-to-content edges where a hard cut reads as accidental, not solid-to-solid color changes between two intentional sections.
- **A transparent-over-Hero floating Nav state — tried in full, reverted entirely, not just tweaked.** The idea: Nav goes fully transparent while positioned over Hero's photo (mono Logo via `brightness-0 invert`, per-element drop-shadow/text-shadow on icons and links instead of a background panel — a `.photo-text-scrim-top` panel was tried first and dropped for reading as a stark band even at matching color), reverting to the normal solid `bg-warm-ivory/95` bar once scrolled past. This went through three real rounds of device-only bugs — a load-timing race in the scroll-position logic, a genuine WebKit fixed-position text-rendering quirk, and (the actual root cause of the worst symptom) the hero container having no CSS-only height fallback, so on a slow connection the transparent header had nothing but blank page background behind it until both the photo and a JS fix-up landed. None of these reproduced in this repo's Chromium-based testing tooling even once, which is exactly what made them hard to fix with any confidence. **Akash's call after that: stop chasing it, use the one reliable solid header everywhere** (see Nav.tsx) — same treatment every other section already had, now used for Hero too instead of introducing a second, fancier state. If this gets revisited, budget real device time for it from the start; there's no way to fix these blind.

- **Recipe:** `.photo-text-scrim` in `globals.css` — solid Espresso at the text-anchored edge fading to fully transparent, built from the locked `--color-espresso` token (not a new color). Apply this class to a positioned div behind the text rather than hand-deriving a gradient per use. Reserve this for a genuinely dense text block that needs real backing (Hero's headline/subhead/trust-strip/CTAs) — see the reverted attempts above for where it's the wrong call, including for Nav, which doesn't use any variant of this pattern any more.
- **Contrast requirement:** whatever technique is used must hold up against the *worst-case* photo it could be shown over — if the photo rotates (carousel) or could be swapped later (CMS-editable), don't tune it against just one photo you happened to be looking at.
- **Focal-point / cropping requirement — learned the hard way 2026-08-29:** object-position's Y axis only does anything on a photo *taller* than the box's own aspect ratio. This was verified against the hero box as it existed that day — forced to an extremely tall ~0.5 (viewport-height-locked) mobile aspect ratio, taller than nearly any ordinary photo, so cover-fit was height-constrained for every photo in rotation and Y had zero visible effect (confirmed directly: swapping a live photo's Y from 10% to 50% produced a pixel-identical render). **That premise changed later the same day** — Hero's mobile photo block is no longer viewport-height-locked, just a plain `h-[62vh]` block (see the layout note below), a less extreme aspect ratio — so Y may matter again for some photos now. Re-verify with the same live-swap technique before assuming it's still inert. X still matters regardless and needs a per-photo value, not one blanket rule (`team-itero-scan.jpg` in `content.ts` is the example: she's in profile on the left third of her own frame, so hers stays left instead of following the others' rightward bias). If a photo has real empty headroom above the subject (measure it — a guide-line overlay at 5% increments is fast and precise), the most reliable fix is still a pre-cropped source variant, same as `archana-candid-crop.jpg` / `archana-crop.webp` / etc.
- **When to use the overlay vs. the solid-panel pattern:** use the overlay when the photo itself is the trust signal being led with (a hero, a section whose whole point is "here's the real space/person"). Keep the solid-panel pattern (TrustBlock's Archana bio card, BookingBlock's photo, etc.) where the photo is supporting evidence next to independently-important text/data — those sections aren't cluttered in the same way the old hero was, and don't need converting just for consistency's sake.

## Hero Mobile Layout — Not Viewport-Height-Locked (Locked, 2026-08-29)

**Decision (LOCKED): Hero's mobile layout is normal document flow — a photo block sized with a plain `vh` unit, followed by a text block (own solid Espresso background) overlapping it by a modest fixed margin. Not an exact-one-viewport-tall container with the text absolutely pinned to its computed bottom edge.**

The exact-viewport-tall version (tried first, several implementations across several commits: JS `visualViewport.height`, then CSS `100dvh`) caused a fourth and final real-device bug on top of the three Nav floating-state ones above: Safari and Chrome on the *same phone* measured the visible viewport differently enough that the CTA row (bottom-pinned to the computed container height) was reachable on one and required scrolling past what looked like the end of the page on the other. Akash's ask after that: make it dynamically show the nav bar and CTA "no matter what," not "once the height math is finally right."

Normal flow removes the failure mode structurally instead of computing around it: the text block's height is determined by its own content, and the browser lays it out after the photo, full stop — there's no container height to get wrong, so the CTA can't end up off-screen due to a bad calculation. The photo's `vh`-based height and the text block's `-mt-*` overlap are tuned so the whole thing *usually* still fits one screen without scrolling on common phone sizes, but being imprecise here only ever costs a few pixels of scroll on an unusually short device — never a missing button, which the exact-height approach couldn't promise no matter how many times its height calculation was rewritten.

**First pass at the overlap amount was also a real bug worth recording:** a big negative margin (-mt-56) with no background of its own, betting the text would always stay short enough to remain within the photo's scrim. On a short screen the trust-strip and CTA extended past that overlap onto the plain page background — warm-ivory text on a warm-ivory page, invisible, not just low-contrast. Fixed by giving the text block its own unconditional `bg-espresso` and pulling the overlap back to a modest `-mt-20` — contrast no longer depends on tuning the overlap to exactly match content height.
