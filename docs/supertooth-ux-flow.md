...
First used in Hero.tsx's mobile layout (2026-08-29), replacing a stacked photo-block + solid-Espresso-panel layout that read as cluttered/two-screens-stacked on review. Extended the same day to Nav.tsx (transparent floating header + top scrim + mono logo while positioned over Hero's photo, see Nav.tsx and Logo.tsx) and to TrustBlock.tsx's top edge (a short fade blending Hero's solid bottom edge into TrustBlock's lighter Sand tint, instead of an instant flat-color cut) — together these were Akash's "single clean bleed, not stacked white/photo/white bands" call, not three separate one-off changes. Desktop's side-by-side split is a separate, already-working pattern and isn't affected by any of this.
