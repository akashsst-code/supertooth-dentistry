...
First used in Hero.tsx's mobile layout (2026-08-29), replacing a stacked photo-block + solid-Espresso-panel layout that read as cluttered/two-screens-stacked on review. Extended the same day to Nav.tsx (transparent floating header + top scrim + mono logo while positioned over Hero's photo, see Nav.tsx and Logo.tsx) — Akash's "single clean bleed, not stacked white/photo/white bands" call. Desktop's side-by-side split is a separate, already-working pattern and isn't affected by any of this.

A gradient fade was also tried at the Hero/TrustBlock seam itself (softening the handoff from Hero's solid bottom edge into TrustBlock's lighter Sand tint) and reverted the same day per Akash's explicit follow-up — a crisp flat-color boundary read as a deliberate, clean section break; the gradient read as muddy and indecisive. Not every section transition on this page needs to bleed — only photo-to-content edges where a hard cut reads as accidental, not solid-to-solid color changes between two intentional sections.

- **Recipe:** `.photo-text-scrim` (bottom-anchored) / `.photo-text-scrim-top` (top-anchored) in `globals.css` — solid Espresso at the text-anchored edge fading to fully transparent, built from the locked `--color-espresso` token (not a new color). Apply the matching class to a positioned div behind the text/nav rather than hand-deriving a gradient per use, so every use gets an identical scrim.
- **Recolored logo requirement:** a fixed raster logo (like this site's tooth mark) can't be recolored per-pixel via CSS to guarantee contrast against an arbitrary photo — Logo.tsx's `mono` prop works around this with a `brightness-0 invert` filter (flattens it to a white silhouette, trading away the Terracotta sparkle's color while floating). If a future full-bleed-photo section needs a logo or icon on top of it, budget for the same kind of light/mono variant rather than assuming the normal colored asset will stay legible.
