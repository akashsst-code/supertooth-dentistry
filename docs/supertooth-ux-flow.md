...
Two adjacent ideas were tried the same day and reverted after Akash's follow-up review — both are worth recording so they aren't re-tried the same way later:
...
- A `.photo-text-scrim-top` panel behind Nav's floating header (same recipe as Hero's own scrim, mirrored) — reverted, the panel itself read as a stark band sitting on the photo rather than merging into it, even at matching color. Nav's sparse content (a small logo, two icon buttons, a few links) doesn't need a background panel to stay legible the way Hero's dense text block does — see "Nav's floating elements" below for what replaced it.

- **Recipe:** `.photo-text-scrim` in `globals.css` — solid Espresso at the text-anchored edge fading to fully transparent, built from the locked `--color-espresso` token (not a new color). Apply this class to a positioned div behind the text rather than hand-deriving a gradient per use. Reserve this for a genuinely dense text block that needs real backing (Hero's headline/subhead/trust-strip/CTAs) — see the two reverted attempts above for where it's the wrong call.
- **Nav's floating elements (no scrim panel):** Nav.tsx's transparent-over-hero header uses per-element shadows instead — Logo.tsx's `mono` prop pairs `brightness-0 invert` with Tailwind's `drop-shadow-[...]` (composes correctly via Tailwind's filter-variable system; a plain custom `filter` CSS rule would silently overwrite the invert instead of combining with it), and nav links/icon buttons get `.text-shadow-photo` / `drop-shadow-[...]` respectively. This keeps the photo fully undimmed everywhere except right behind each glyph, which is what "merges into the photo" actually requires for small sparse elements — a background panel is the wrong tool here even at matching color.
- **Recolored logo requirement:** a fixed raster logo (like this site's tooth mark) can't be recolored per-pixel via CSS to guarantee contrast against an arbitrary photo — see Logo.tsx's `mono` prop above. If a future full-bleed-photo section needs a logo or icon on top of it, budget for the same kind of light/mono variant rather than assuming the normal colored asset will stay legible.
- **Contrast requirement:** whatever technique is used (scrim panel or per-element shadow) must hold up against the *worst-case* photo it could be shown over — if the photo rotates (carousel) or could be swapped later (CMS-editable), don't tune it against just one photo you happened to be looking at.
- **Focal-point / cropping requirement — learned the hard way 2026-08-29:** object-position's Y axis only does anything on a photo *taller* than the box's own aspect ratio. This hero box is extremely tall (~0.5 mobile), taller than nearly any ordinary photo — so for every photo actually in rotation here, cover-fit is height-constrained and Y has zero visible effect (verified directly: swapping a live photo's Y from 10% to 50% produced a pixel-identical render). X still matters and needs a per-photo value, not one blanket rule (`team-itero-scan.jpg` in `content.ts` is the example: she's in profile on the left third of her own frame, so hers stays left instead of following the others' rightward bias). If a photo has real empty headroom above the subject (measure it — a guide-line overlay at 5% increments is fast and precise), the only actual fix is a pre-cropped source variant, same as `archana-candid-crop.jpg` / `archana-crop.webp` / etc. — not a bigger Y number.
