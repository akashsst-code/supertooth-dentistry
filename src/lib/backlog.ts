...
export const backlog: BacklogItem[] = [
  ...
  },
  {
    ...
    scope: [
      ...
      "DONE (2026-08-31): the token has a real consumer — the 'Dental emergency' link in BookingBlock.tsx. Went through FOUR treatments in one day, chasing the wrong variable more than once: (1) full-width solid Hero button at #A32E1F — 'too blaring'; (2) muted the hex to #9C5240 as a small pill next to Book Appointment's pill — read as 'the same color' as Book (both are inherently warm earthy reds under the locked-palette constraint; hue alone doesn't separate them); (3) dropped to plain light text (a since-removed derived token) to escape the color clash — now 'hidden', too quiet to register as an action; (4) current and final: back to the ORIGINAL undiluted #A32E1F, as a small compact badge (not a pill matching Book's size) on its own line, not adjacent to Book. This matches how NHS/GOV.UK-style services actually handle a reserved emergency red — undiluted color, restrained through size/rarity of use, not through muting the hue. Items 7 (/emergency) and 45 (sitewide reachability) are still not built, so this is the token proven at its one current call site, not full coverage",
    ...
    acceptance: [
      ...
      "Emergency action unmistakably distinct from booking CTAs at 375px — DONE for the BookingBlock badge: undiluted `bg-alert` fill, deliberately smaller than the Book/Call pills and on its own line rather than adjacent to Book, which is what actually fixed the earlier 'reads as the same color' complaint — a pill-vs-pill size/position problem, not purely a hue problem; still PENDING sitewide once item 45 lands more entry points",
  ...
  // ══════════════════════════════════════════════════════════════════
  {
    ...
    scope: [
      ...
      "REVISED 2026-08-31, three passes same day: (1) secondary pair first lived directly under BookingBlock's 'Quick actions' pills — clutter (3-4 things stacked in one small area) and the Dental Emergency pill read as the same color as the terracotta Book pill next to it; (2) moved to a quiet inline line below Office hours/Location and dropped Emergency to plain tinted text — fixed the clutter/color-clash but the emergency link then read as 'hidden'; (3) current: same quiet line placement (that part worked), but Dental Emergency is a small compact `bg-alert` badge — undiluted color, small size, own line rather than beside Book. See item 46 for the token history.",
