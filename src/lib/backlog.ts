...
export const backlog: BacklogItem[] = [
  ...
  },
  {
    ...
    evidence:
      "content.ts's own 2026-09-01 comment at the `services` array (\"Swapped all 4 for tasteful, patient-facing stock photography... same Unsplash hotlink pattern as `offers` below\") documents both the cause and the two affected arrays firsthand — not inferred.\n\n2026-09-03: the \"Jaw clenching, TMJ and headaches\" service card added by item 66 has NO photograph at all — it renders the icon-tile fallback. That makes it the first and most visible gap this item should close, ahead of swapping the four Unsplash service photos, since the others at least look like the practice they claim to be.",
  ...
  },
  {
    ...
    evidence:
      ...
      "\n\n2026-09-02: fixed. Every failing text-espresso/20–65 callsite on the 8 patient-facing routes (Nav.tsx, Footer.tsx, AppointmentForm.tsx, Logo.tsx, LocationMapSection.tsx, BenefitsGlossary.tsx, and each page's breadcrumb/last-updated/address line) raised to text-espresso/70 — the lowest opacity that clears 4.5:1 against #faf8f4 (measured 4.77:1), confirmed against the site's actual mobile-first text-xs/text-sm size (14px), not the large-text 3:1 exception. The tel: link in AppointmentForm.tsx switched from text-terracotta (3.86:1) to text-terracotta-dark (5.32:1). Re-running the harness's own axe-core command against all 8 routes surfaced 3 more real color-contrast violations the original evidence hadn't named — text-warm-ivory/50 on bg-espresso in BookingBlock.tsx (3 labels, 4.21:1) and TestimonialsSection.tsx (3.51:1 on the blended card background), plus text-terracotta on bg-espresso in BookingBlock's 'Visit us' label (3.04:1, and full-opacity terracotta on espresso tops out at 3.04:1 regardless of opacity, so it needed a token swap rather than a bump) — fixed the opacity instances to text-warm-ivory/70 (5.23–6.65:1) and switched the terracotta label to text-warm-ivory/70 to match its sibling eyebrow labels. Verified with `npx @axe-core/cli` at 375×812 against all 8 routes post-fix: 0 color-contrast violations everywhere. No --color-* base token value touched (git diff on globals.css is empty) — every fix was a callsite's opacity or token choice, per the acceptance criteria.",
    dependsOn: null,
    outOfScope:
  ...
  },
  {
    ...
    evidence:
      "Practice tier: the six items come directly from the 2026-09-03 review, where Akash listed what had to be checked before publication. Internal: content.ts already carries the admission that the AACE/AAFE acronym is unresolved — the risk here is documented in the codebase, not hypothetical. Standards tier: professional credentials are the highest-consequence category of claim on a healthcare site; the ADA's veracity principle and the FTC's substantiation rule both apply, and a wrong credential acronym is not a typo but a misstatement of qualification. This is the specific instance item 2's sweep and item 31's pre-launch gate are supposed to catch; it is logged here so it cannot fall between the two.\n\n2026-09-03, four of the six answered by Akash directly in chat, recorded here so the next pass starts from evidence. (1) BOTOX CREDENTIAL: \"aafe for botox\" — American Academy of Facial Esthetics, which matches both the 2026-09-02 credential supply and the review's own written wording, so the \"AACE\" reading in the 2026-09-03 review notes was a transcription artifact. content.ts now publishes \"AAFE Trained and Certified Botox Provider\" with the academy named in `detail`. (2) AVAILABILITY: his framing was \"can we say something that's a culmination of same day when we have, or ASAP, same-day emergency, we work to make space\" — a process, not a guarantee, and it matches the practice's own live-site wording (\"in most cases, emergency visits are accommodated on the day you call as soon as we can schedule an appointment\") plus its on-call line. Item 62's removed row is replaced on that basis, and the emergency FAQ answer is restored to the same hedge. Note what is still NOT claimed: same-day ROUTINE appointments, which is the claim he called inaccurate. (3) BOTOX FOR TMJ/HEADACHES: \"yes, we do, and want to highlight, even in earlier section 2\" — so it ships both as a differentiator row and as the fifth service category, and item 65's held-back constraint is released. (4) SCOPE: \"bring all in\" on the cosmetic and restorative question — everything on the practice's live site is in-house, so root canals, extractions, implant restorations, dentures, metal-free fillings, whitening and cosmetic consultations are all named on the cards.\n\nSTILL OPEN, and why this item stays launch-blocking: (a) the AAFE certificate itself has not been put on file — this is the owner's confirmation, which is the tier used for capability claims, not the document a credential needs; (b) \"Gold Provider\" — the AACA Gold Status row is live off Akash's 2026-09-02 supply with no certificate behind it either; (c) the carrier list behind \"in-network with most plans\" has not been re-checked for currency; (d) the exact downtown/Queen Anne location wording is unanswered; (e) INSTANT ONLINE BOOKING — he answered \"yes we have instant\", but the practice's own /appointment page is a request form (its Forms link points at a Tab32 short URL), and no instant-booking URL has been supplied. Nothing on the site claims instant booking until that link exists and resolves to a real scheduler, because a booking claim whose CTA opens a request form is the exact failure mode item 62 was opened for.",
    dependsOn: null,
    outOfScope:
  ...
  },
  {
    ...
    evidence:
      "Practice tier, direct: the ordering and its rationale are Akash's, stated twice in the session — insurance-driven patients first, modern/digital second. Usability tier: NN/g's eyetracking work finds attention concentrated in the first screenfuls, which is the argument for spending the first row on the qualifier most patients actually filter by rather than on a capability claim. Internal: docs/supertooth-webflow-build-spec.md Section 1 makes new-patient conversion the goal, and insurance participation is the most common disqualifier in the researched patient scenarios.\n\n2026-09-03 implementation, PARTIAL by design: in-network now leads and a modern/digital row is second, which is the ordering half of this item, done. The row set is three, not the seven the review named — in-network, modern digital dentistry, same-day crowns. The other four (verified Botox qualification, a verified convenience benefit, care/skill/choice, downtown location) each need an answer from item 63, which is blocked, and this item's own acceptance rule is that every row traces to a verified fact; three real rows beat seven padded ones. The new modern/digital row asserts nothing new — digital scanning and in-house milling are already published in `services` as real and are the same capability the same-day-crown row rests on. Measured at 375x812 before and after: the trust block is 3.63 screens against 3.54 before, so the reorder cost 0.09 of a screen. Scheduling is still the section's only action. Remaining work is the four gated rows; status stays partial until 63 lands.\n\nSame-day follow-up, 2026-09-03: two more of the review's seven rows landed once Akash answered — the Botox qualification row (\"Botox for jaw pain and headaches — therapeutic Botox, AAFE trained and certified\", which he explicitly asked to be highlighted in this section and not only in the credential badges) and the convenience row (\"We make space when it's urgent — same-day when we have it, as soon as we can when we don't\"), which is item 62's replacement written in his own framing. Five rows now, up from three. The remaining two are care/skill/choice — which item 67 put in the office blurb instead, where the review actually asked for it — and the downtown location line, still unanswered in item 63. Measured at 375x812: the trust block went 3.63 -> 4.07 screens for the two extra rows. Status stays partial only on the location row.\n\nRow order and copy revised on Akash's direct instruction, 2026-09-03: in-network, same-day visits, same-day crowns, modern/digital, Botox. Three specific changes he asked for. (1) The urgency row moves from last to second and is renamed \"Same-day visits\" with the emergency/non-emergency split as its line. (2) The in-network row now names carriers — \"Delta, Premera, Aetna and more — we handle the insurance paperwork\" — derived from `insuranceCarriersHeroTeaser` rather than retyped, which is why both carrier exports moved above `differentiators` in content.ts (a const referenced before its declaration is a TDZ error, not a hoisting convenience). (3) The Botox row is cut to two lines total at 375px — \"Botox for TMJ and headaches\" over \"AAFE trained and certified.\" — since the longer title wrapped and made the row four lines deep. Measured at 375x812 after: five rows, every title one line, trust block 4.0 screens. One judgement call flagged rather than taken quietly: modern/digital keeps its place AHEAD of the Botox row, because the review's stated rationale ranked it the second differentiator overall and fourth is as high as that survives once same-day visits and crowns are inserted above it. Flipping the last two rows is a one-line change if the Botox highlight should outrank it.",
    dependsOn: "62, 63",
    outOfScope:
