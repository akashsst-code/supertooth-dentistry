...
export const backlog: BacklogItem[] = [
  ...
  },
  {
    ...
    scope: [
      ...
      "DONE (2026-08-31): the token has a real consumer — the 'Dental emergency' quick-action link in BookingBlock.tsx, a small solid `bg-alert` pill (ivory text + MedicalCrossIcon on the fill), not a full-width block. Item 48 briefly consumed it as a full-width solid Hero button; reverted per Akash's 'don't overwhelm, keep emergency lower' call. Note: BookingBlock has an espresso background, and the alert token's contrast numbers were only ever checked against warm-ivory — using it as plain text color directly on espresso measured ~2:1 (fails AA) during this pass, caught and fixed by keeping it a self-contained pill (ivory-on-alert contrast holds regardless of the surrounding section's background). Items 7 (/emergency) and 45 (sitewide reachability) are still not built, so this is the token proven at its one current call site, not full coverage",
    ...
    acceptance: [
      ...
      "AA verified on every surface pairing it appears in — DONE: the real consumer (BookingBlock's emergency pill) uses ivory text ON the alert fill (5.36:1), a self-contained pairing that holds regardless of the surrounding section's background. Caught and avoided the failure mode during this pass — the token as plain text color directly on BookingBlock's espresso background measured ~2:1 — by keeping it a pill rather than inline text. Still verify a solid-fill-on-espresso case (e.g. a bordered chip) if a future consumer wants that treatment instead",
      "Emergency action unmistakably distinct from booking CTAs at 375px — DONE for the BookingBlock quick-action link (icon + text label, visually distinct from the terracotta Book pill above it); still PENDING sitewide once item 45 lands more entry points",
  ...
  // ══════════════════════════════════════════════════════════════════
  {
    ...
    where: "Hero (Book + Call, unchanged) · BookingBlock.tsx Quick actions (New Patient + Dental Emergency, secondary)",
    scope: [
      ...
      "DONE (2026-08-31): three real, static-Link entry points exist — Book Appointment (Hero, primary) → /contact, New Patient (BookingBlock, secondary) → /insurance-new-patients, Dental Emergency (BookingBlock, secondary) → /contact. No branching logic anywhere. Emergency link consumes item 46's `--color-alert` token (revised same day to a less saturated #9C5240 after direct feedback the first value was 'too blaring')",
