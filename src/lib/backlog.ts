...
export const backlog: BacklogItem[] = [
  ...
  },
  {
    ...
    scope: [
      ...
      "DONE (2026-08-31): the token has a real consumer — the 'Dental emergency' link in BookingBlock.tsx. Went through three treatments in one day: (1) full-width solid Hero button, reverted per 'don't overwhelm, keep emergency lower'; (2) a small solid `bg-alert` pill next to Book Appointment's pill — Akash flagged this and the Book pill 'read as the same color' sitting adjacent (both are inherently warm earthy reds under the locked-palette constraint, so hue alone doesn't separate them); (3) current: plain text, no fill, using a new derived token `--color-alert-light` (a lighter-lightness tint of the same hue, 4.62:1 on espresso — AA as text, where base `--color-alert` only hits ~2:1). Distinction now comes from tint + icon, not a background fill that reads as a button. Items 7 (/emergency) and 45 (sitewide reachability) are still not built, so this is the token proven at its one current call site, not full coverage",
    ...
    acceptance: [
      ...
      "AA verified on every surface pairing it appears in — DONE: the real consumer (BookingBlock's emergency link) uses `--color-alert-light` as plain text on espresso, 4.62:1 — AA. Two failure modes caught and fixed during this pass: the base `--color-alert` as plain text on espresso measured ~2:1, and a solid `bg-alert` pill treatment (correct contrast) still wasn't distinct enough at a glance from the terracotta Book pill sitting next to it — see decision above",
      "Emergency action unmistakably distinct from booking CTAs at 375px — DONE for the BookingBlock link: no background fill at all now (avoids pill-vs-pill color confusion), `--color-alert-light` text + MedicalCrossIcon, visually distinct in both hue-lightness and treatment from the terracotta Book pill above it; still PENDING sitewide once item 45 lands more entry points",
  ...
  // ══════════════════════════════════════════════════════════════════
  {
    ...
    where: "Hero (Book + Call, unchanged) · BookingBlock.tsx, below Office hours/Location (New Patient + Dental Emergency, one quiet inline line)",
    scope: [
      ...
      "REVISED 2026-08-31 (same day, second pass): the secondary pair first lived directly under BookingBlock's 'Quick actions' pills — Akash flagged it as clutter (effectively 3-4 things stacked in one small area) and flagged the Dental Emergency pill and the terracotta Book pill as reading like the same color next to each other. Moved to their own quiet inline line below Office hours/Location, no label header, both plain text/icon (no background fill) — reads as a footnote, not a fourth competing section. Emergency's distinction now comes from item 46's new `--color-alert-light` derived token (see item 46) rather than a colored pill.",
